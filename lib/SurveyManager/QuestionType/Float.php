<?php
/**
 * SurveyManager.
 *
 * @copyright Zikula Development Team
 * @license http://www.gnu.org/licenses/lgpl.html GNU Lesser General Public License
 * @package SurveyManager
 * @author Zikula Development Team <info@zikula.org>.
 * @link http://zikula.org
 * @link http://zikula.org
 * @version Generated by ModuleStudio 0.5.5 (http://modulestudio.de) at Tue Aug 21 20:13:14 CEST 2012.
 */

/**
 * Question type class for float values.
 */
class SurveyManager_QuestionType_Float extends SurveyManager_AbstractQuestionType
{
    /**
     * Returns the question type code.
     */
    public function getID()
    {
        return 'float';
    }

    /**
     * Returns the question type name.
     */
    public function getName()
    {
        return $this->__('Decimal value');
    }

    /**
     * Returns the question type category.
     */
    public function getGroup()
    {
        return $this->__('Numbers');
    }

    /**
     * Returns the editing ui component(s).
     */
    public function getEditInput($question, $response, $context = '')
    {
        $questionId = $question['id'];
        $cssClass = $question['required'] ? 'required ' : '';
        $output = '<input class="' . $cssClass . 'validate-number" type="text" id="responses_' . $questionId . '" name="responses[' . $questionId . ']" size="4" value="' . DataUtil::formatForDisplay($response) . "\" />\n";
        if ($question['required']) {
            $output .= '<p id="advice-required-responses_' . $questionId . '" class="custom-advice" style="display: none">' . $this->__('Please enter a value for this field.') . "</p>\n";
        }
        $output .= '<p id="advice-validate-number-responses_' . $questionId . '" class="custom-advice" style="display: none">' . $this->__('Please only enter a valid decimal value in this field.') . "</p>\n";
        if ($context == 'block_survey_display') {
            $output .= '<br />';
        }
        return $output;
    }
}
