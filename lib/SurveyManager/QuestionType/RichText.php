...
/**
 * Question type class for styled text.
 */
class SurveyManager_QuestionType_RichText extends SurveyManager_AbstractQuestionType
{
    /**
    ...
    {
        $value = $this->linebreaks($value);
        $value = stripslashes($value);
        $value = DataUtil::formatForDisplayHTML($value);
        return $value;
    }
    ...
    public function getEditInput($question, $response, $context = '')
    {
        ...
        $output = '<textarea class="' . $cssClass . ' simple_editor" type="text" id="responses_' . $questionId . '" name="responses[' . $questionId . ']" rows="10" cols="57">' . DataUtil::formatForDisplay($response) . "</textarea>\n";
