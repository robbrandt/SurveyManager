 ...
 */
class SurveyManager_Controller_Base_User extends Zikula_AbstractController
{
    /**
     * Post initialise.
     *
    ...
    public function view()
    {
        ...
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        ...
        $modelHelper = new SurveyManager_Util_Model($this->serviceManager);
    ...
    public function processPage()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'processPage');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        ...
        // return template
        return $this->view->fetch('user/processPage.tpl');
    }
    ...
    public function clearResponses()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'clearResponses');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_OVERVIEW), LogUtil::getErrorMsgPermission());
        ...
        // return template
        return $this->view->fetch('user/clearResponses.tpl');
    }
    ...
    public function thankYou()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'thankYou');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_OVERVIEW), LogUtil::getErrorMsgPermission());
    ...
    public function viewFeatured()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'viewFeatured');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_OVERVIEW), LogUtil::getErrorMsgPermission());
    ...
    public function displayFeatured()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'displayFeatured');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_OVERVIEW), LogUtil::getErrorMsgPermission());
     ...
     * @return mixed Output.
     */
    public function detailFeatured()
    {
        $controllerHelper = new SurveyManager_Util_Controller($this->serviceManager);
        
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'detailFeatured');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_OVERVIEW), LogUtil::getErrorMsgPermission());
    ...
    public function viewResponse()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'viewResponse');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_OVERVIEW), LogUtil::getErrorMsgPermission());
        ...
        // return template
        return $this->view->fetch('user/viewResponse.tpl');
    }
