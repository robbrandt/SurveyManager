 ...
 */
class SurveyManager_Controller_Base_Admin extends Zikula_AbstractController
{
    /**
     * Post initialise.
     *
     ...
     * @return mixed Output.
     */
    public function treeOp()
    {
        $controllerHelper = new SurveyManager_Util_Controller($this->serviceManager);
        
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'admin', 'action' => 'treeOp');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_ADMIN), LogUtil::getErrorMsgPermission());
        ...
        // return template
        return $this->view->fetch('admin/treeOp.tpl');
    }
    ...
    public function featured()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'admin', 'action' => 'featured');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_ADMIN), LogUtil::getErrorMsgPermission());
    ...
    public function export()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'admin', 'action' => 'export');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_ADMIN), LogUtil::getErrorMsgPermission());
    ...
    public function wizard()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = $this->request->query->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'admin', 'action' => 'wizard');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerAction', $utilArgs);
        }
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucwords($objectType) . ':', '::', ACCESS_ADMIN), LogUtil::getErrorMsgPermission());
    ...
    public function handleSelectedEntries()
    {
        ...
        $workflowHelper = new SurveyManager_Util_Workflow($this->serviceManager);
    ...
    public function config()
    {
        ...
        // Execute form using supplied template and page event handler
        return $view->execute($templateName, new SurveyManager_Form_Handler_Admin_Config());
    }
}
