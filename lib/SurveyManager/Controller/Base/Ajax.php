 ...
 */
class SurveyManager_Controller_Base_Ajax extends Zikula_Controller_AbstractAjax
{
    ...
    public function getItemListFinder()
    {
        ...
        if ($this->request->isPost() && $this->request->request->has('ot')) {
            $objectType = $this->request->request->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        ...
        $entityClass = 'SurveyManager_Entity_' . ucfirst($objectType);
    ...
    public function checkForDuplicate()
    {
        ...
        $objectType = $postData->filter('ot', 'survey', FILTER_SANITIZE_STRING);
        $controllerHelper = new SurveyManager_Util_Controller($this->serviceManager);
    ...
    public function toggleFlag()
    {
        ...
        if ($id == 0
            || ($objectType != 'survey')
        || ($objectType == 'survey' && !in_array($field, array('archived')))
        ) {
            return new Zikula_Response_Ajax_BadData($this->__('Error: invalid input.'));
        }
