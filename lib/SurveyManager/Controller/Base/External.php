 ...
 */
class SurveyManager_Controller_Base_External extends Zikula_AbstractController
{
    /**
     * Post initialise.
     *
    ...
    {
        PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('SurveyManager'));
    
        $getData = $this->request->query;
        $controllerHelper = new SurveyManager_Util_Controller($this->serviceManager);
    
        $objectType = $getData->filter('objectType', 'survey', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'external', 'action' => 'finder');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controller', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerType', $utilArgs);
        }
    
        $this->throwForbiddenUnless(SecurityUtil::checkPermission('SurveyManager:' . ucwords($objectType) . ':', '::', ACCESS_COMMENT), LogUtil::getErrorMsgPermission());
        ...
        $view = Zikula_View::getInstance('SurveyManager', false);
