 ...
 */
class SurveyManager_Form_Handler_Admin_Response_Base_Edit extends SurveyManager_Form_Handler_Admin_Edit
{
    /**
     * Pre-initialise hook.
     *
    ...
    public function preInitialize()
    {
        ...
        $this->objectTypeCapital = 'Response';
        $this->objectTypeLower = 'response';
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        if ($this->mode == 'create') {
            ...
            if (!$modelHelper->canBeCreated($this->objectType)) {
                LogUtil::registerError($this->__('Sorry, but you can not create the response yet as other items are required which must be created before!'));
        ...
        $this->relationPresets['survey'] = FormUtil::getPassedValue('survey', '', 'GET');
        if (!empty($this->relationPresets['survey'])) {
            $relObj = ModUtil::apiFunc($this->name, 'selection', 'getEntity', array('ot' => 'survey', 'id' => $this->relationPresets['survey']));
            if ($relObj != null) {
                $relObj->addResponses($entity);
            }
        }
    ...
    protected function getRedirectCodes()
    {
        ...
        // admin list of surveys
        $codes[] = 'adminViewSurvey';
        // user list of surveys
        $codes[] = 'userViewSurvey';
        // user display page of treated survey
        $codes[] = 'userDisplaySurvey';
    ...
    {
        // redirect to the list of responses
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                        if ($this->mode == 'create') {
                            $message = $this->__('Done! Response created.');
                        } else {
                            $message = $this->__('Done! Response updated.');
                        }
                        break;
            case 'delete':
                        $message = $this->__('Done! Response deleted.');
                        break;
            default:
                        $message = $this->__('Done! Response updated.');
                        break;
    ...
    protected function getRedirectUrl($args)
    {
        ...
        switch ($this->returnTo) {
            ...
            case 'userDisplay':
                ...
                return $this->getDefaultReturnUrl($args);
            case 'adminViewSurvey':
                return ModUtil::url($this->name, 'admin', 'view',
                                         array('ot' => 'survey'));
            case 'userViewSurvey':
                return ModUtil::url($this->name, 'user', 'view',
                                         array('ot' => 'survey'));
            case 'userDisplaySurvey':
                if (!empty($this->relationPresets['survey'])) {
                    return ModUtil::url($this->name, 'user', 'display', array('ot' => 'survey', 'id' => $this->relationPresets['survey']));
                }
                return $this->getDefaultReturnUrl($args);
            default:
                return $this->getDefaultReturnUrl($args);
