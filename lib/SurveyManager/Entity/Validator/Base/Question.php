 ...
 *
 * This is the base validation class for question entities.
 */
class SurveyManager_Entity_Validator_Base_Question extends SurveyManager_Validator
{
    /**
    ...
    public function validateAll()
    {
        ...
        if (!$this->isStringNotLongerThan('questionType', 50)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('question type', 50), $dom);
            return $errorInfo;
        }
        if (!$this->isValidBoolean('required')) {
            $errorInfo['message'] = __f('Error! Field value must be a valid boolean (%s).', array('required'), $dom);
            return $errorInfo;
        }
     ...
     *
     * This method determines if there already exist questions with the same question.
     *
