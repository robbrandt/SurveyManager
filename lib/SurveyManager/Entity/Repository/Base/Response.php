 ...
 *
 * This is the base repository class for response entities.
 */
class SurveyManager_Entity_Repository_Base_Response extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'ipAddress';
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('response', 'workflowState');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['featuredItems'] = $booleanSelectorItems;
            }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['survey'] = isset($this->controllerArguments['survey']) ? $this->controllerArguments['survey'] : FormUtil::getPassedValue('survey', 0, 'GET');
        ...
        $parameters['featured'] = isset($this->controllerArguments['featured']) ? $this->controllerArguments['featured'] : FormUtil::getPassedValue('featured', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('SurveyManager_Entity_Response', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('SurveyManager_Entity_Response', 'tbl')
     ...
     *
     * @return array|SurveyManager_Entity_Response retrieved data array or SurveyManager_Entity_Response instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved SurveyManager_Entity_Response instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('SurveyManager', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'response', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('featured'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved responses only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= 'tbl.ipAddress LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.ipAddress LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.responseTimestamp = \'' . $fragment . '\'';
        }
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numResponses';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('SurveyManager_Entity_Response', 'tbl');
     ...
     * @param int    $excludeId  Id of responses to exclude (optional).
     *
    ...
    {
        $selection = ', tblSurvey, tblAnswers';
    ...
    {
        $qb->leftJoin('tbl.survey', 'tblSurvey');
