 ...
 *
 * This is the base repository class for survey entities.
 */
class SurveyManager_Entity_Repository_Base_Survey extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'thankYouTitle';
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new SurveyManager_Util_ListEntries(ServiceUtil::getManager());
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('survey', 'workflowState');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['useCaptchaItems'] = $booleanSelectorItems;
                $templateParameters['archivedItems'] = $booleanSelectorItems;
                $templateParameters['templateItems'] = $booleanSelectorItems;
            }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['useCaptcha'] = isset($this->controllerArguments['useCaptcha']) ? $this->controllerArguments['useCaptcha'] : FormUtil::getPassedValue('useCaptcha', '', 'GET');
        $parameters['archived'] = isset($this->controllerArguments['archived']) ? $this->controllerArguments['archived'] : FormUtil::getPassedValue('archived', '', 'GET');
        $parameters['template'] = isset($this->controllerArguments['template']) ? $this->controllerArguments['template'] : FormUtil::getPassedValue('template', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('SurveyManager_Entity_Survey', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('SurveyManager_Entity_Survey', 'tbl')
     ...
     *
     * @return array|SurveyManager_Entity_Survey retrieved data array or SurveyManager_Entity_Survey instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved SurveyManager_Entity_Survey instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     *
     * @return SurveyManager_Entity_Survey retrieved instance of SurveyManager_Entity_Survey
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('SurveyManager', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'survey', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('useCaptcha', 'archived', 'template'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'SurveyManager') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved surveys only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.thankYou LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.thankYouTitle LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.thankYouAlternativeUrl = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.responseSubject LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.confirmationSubject LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.confirmationBody LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.addition1 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.addition2 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.addition3 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.addition4 LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.addition5 LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.name LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.maxPerIp = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.maxPerUserId = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.responseSubject LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.confirmationSubject LIKE \'%' . $fragment . '%\'';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numSurveys';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('SurveyManager_Entity_Survey', 'tbl');
     ...
     * @param int    $excludeId  Id of surveys to exclude (optional).
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('SurveyManager', 'showOnlyOwnEntries', 0), 'GETPOST');
    ...
    {
        $selection = ', tblPages, tblResponses';
    ...
    {
        $qb->leftJoin('tbl.pages', 'tblPages');
        $qb->leftJoin('tbl.responses', 'tblResponses');
