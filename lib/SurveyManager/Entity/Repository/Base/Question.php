 ...
 */
class SurveyManager_Entity_Repository_Base_Question extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'questionType';
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('question', 'workflowState');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['requiredItems'] = $booleanSelectorItems;
            }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['page'] = isset($this->controllerArguments['page']) ? $this->controllerArguments['page'] : FormUtil::getPassedValue('page', 0, 'GET');
        ...
        $parameters['required'] = isset($this->controllerArguments['required']) ? $this->controllerArguments['required'] : FormUtil::getPassedValue('required', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('SurveyManager_Entity_Question', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('SurveyManager_Entity_Question', 'tbl')
     ...
     *
     * @return array|SurveyManager_Entity_Question retrieved data array or SurveyManager_Entity_Question instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved SurveyManager_Entity_Question instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('SurveyManager', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'question', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('required'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.questionType LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.name LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('SurveyManager_Entity_Question', 'tbl');
     ...
     * @param int    $excludeId  Id of questions to exclude (optional).
     *
