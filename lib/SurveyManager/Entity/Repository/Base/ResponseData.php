 ...
 *
 * This is the base repository class for response data entities.
 */
class SurveyManager_Entity_Repository_Base_ResponseData extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'responseValue';
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('responseData', 'workflowState');
            }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['response'] = isset($this->controllerArguments['response']) ? $this->controllerArguments['response'] : FormUtil::getPassedValue('response', 0, 'GET');
        $parameters['question'] = isset($this->controllerArguments['question']) ? $this->controllerArguments['question'] : FormUtil::getPassedValue('question', 0, 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('SurveyManager_Entity_ResponseData', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('SurveyManager_Entity_ResponseData', 'tbl')
     ...
     *
     * @return array|SurveyManager_Entity_ResponseData retrieved data array or SurveyManager_Entity_ResponseData instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved SurveyManager_Entity_ResponseData instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('SurveyManager', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'responseData', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved response datas only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= 'tbl.responseValue LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.responseValue LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.grade = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.correctness = \'' . $fragment . '\'';
        }
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numResponseDatas';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('SurveyManager_Entity_ResponseData', 'tbl');
     ...
     * @param int    $excludeId  Id of response datas to exclude (optional).
     *
    ...
    {
        $selection = ', tblResponse, tblQuestion';
    ...
    {
        $qb->leftJoin('tbl.response', 'tblResponse');
