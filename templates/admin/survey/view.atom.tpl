{* purpose of this template: surveys atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='survey'}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='survey' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='SurveyManager' type='admin' func='view' ot='survey' fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$survey.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='survey'}{/capture}
        <id>{$uniqueID}</id>
