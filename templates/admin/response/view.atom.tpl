{* purpose of this template: responses atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='response'}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='response' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='SurveyManager' type='admin' func='view' ot='response' fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$response.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='response'}{/capture}
        <id>{$uniqueID}</id>
