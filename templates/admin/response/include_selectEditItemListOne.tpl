{* purpose of this template: inclusion template for display of related response in admin area *}
...
<li id="{$idPrefixItem}">
    ...
    <a id="{$idPrefixItem}Edit" href="{modurl modname='SurveyManager' type='admin' func='edit' ot='response' id=$item.id}">{$editImage}</a>
     <a id="{$idPrefixItem}Remove" href="javascript:surveymanRemoveRelatedItem('{$idPrefix}', '{$item.id}');">{$removeImage}</a>
</li>
{/if}
</ul>
