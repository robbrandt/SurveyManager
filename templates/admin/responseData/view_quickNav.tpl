{* purpose of this template: response datas view filter form in admin area *}
{checkpermissionblock component='SurveyManager:ResponseData:' instance='::' level='ACCESS_EDIT'}
{assign var='objectType' value='responseData'}
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="surveyManagerResponseDataQuickNavForm" class="surveymanager-quicknav">
    <fieldset>
        ...
        {if !isset($responseFilter) || $responseFilter eq true}
                <label for="response">{gt text='Responses'}</label>
                {modapifunc modname='SurveyManager' type='selection' func='getEntities' ot='response' useJoins=false assign='listEntries'}
                ...
                {foreach item='option' from=$listEntries}
                    ...
                    <option value="{$entryId}"{if $entryId eq $response} selected="selected"{/if}>{$option->getTitleFromDisplayPattern()}</option>
                {/foreach}
                </select>
        {/if}
        {if !isset($questionFilter) || $questionFilter eq true}
                <label for="question">{gt text='Questions'}</label>
                {modapifunc modname='SurveyManager' type='selection' func='getEntities' ot='question' useJoins=false assign='listEntries'}
                <select id="question" name="question">
                    <option value="">{$lblDefault}</option>
                {foreach item='option' from=$listEntries}
                    {assign var='entryId' value=$option.id}
                    <option value="{$entryId}"{if $entryId eq $question} selected="selected"{/if}>{$option->getTitleFromDisplayPattern()}</option>
                {/foreach}
                </select>
        {/if}
        ...
        {if !isset($sorting) || $sorting eq true}
                ...
                <select id="sortBy" name="sort">
                    ...
                    <option value="responseValue"{if $sort eq 'responseValue'} selected="selected"{/if}>{gt text='Response value'}</option>
                    <option value="grade"{if $sort eq 'grade'} selected="selected"{/if}>{gt text='Grade'}</option>
                    <option value="correctness"{if $sort eq 'correctness'} selected="selected"{/if}>{gt text='Correctness'}</option>
...
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        surveymanInitQuickNavigation('responseData', 'admin');
