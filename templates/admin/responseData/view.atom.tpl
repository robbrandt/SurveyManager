{* purpose of this template: response datas atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='responseData'}{/capture}
    <id>{$uniqueID}</id>
    <updated>{$items[0].updatedDate|default:$smarty.now|dateformat:'%Y-%m-%dT%H:%M:%SZ'}</updated>
{/if}
    <link rel="alternate" type="text/html" hreflang="{lang}" href="{modurl modname='SurveyManager' type='admin' func='main' fqurl=1}" />
...
{foreach item='responseData' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='SurveyManager' type='admin' func='view' ot='responseData' fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$responseData.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='responseData'}{/capture}
        <id>{$uniqueID}</id>
