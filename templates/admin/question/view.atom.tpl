{* purpose of this template: questions atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='question'}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='question' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='SurveyManager' type='admin' func='view' ot='question' fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$question.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='question'}{/capture}
        <id>{$uniqueID}</id>
