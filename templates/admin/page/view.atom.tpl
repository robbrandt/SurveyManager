{* purpose of this template: pages atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='page'}{/capture}
    <id>{$uniqueID}</id>
...
{foreach item='page' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='SurveyManager' type='admin' func='view' ot='page' fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$page.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='SurveyManager' type='admin' func='view' ot='page'}{/capture}
        <id>{$uniqueID}</id>
