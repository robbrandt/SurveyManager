...
<div class="z-formrow">
    <label for="surveyManagerObjectType">{gt text='Object type'}:</label>
        <select id="surveyManagerObjectType" name="objecttype" size="1">
            <option value="survey"{if $objectType eq 'survey'} selected="selected"{/if}>{gt text='Surveys'}</option>
            <option value="page"{if $objectType eq 'page'} selected="selected"{/if}>{gt text='Pages'}</option>
            <option value="question"{if $objectType eq 'question'} selected="selected"{/if}>{gt text='Questions'}</option>
            <option value="response"{if $objectType eq 'response'} selected="selected"{/if}>{gt text='Responses'}</option>
            <option value="responseData"{if $objectType eq 'responseData'} selected="selected"{/if}>{gt text='Response datas'}</option>
        </select>
...
{if $catIds ne null && is_array($catIds)}
    ...
    {foreach key='propertyName' item='propertyId' from=$catIds}
        <div class="z-formrow">
            ...
            &nbsp;
                {selector_category name="`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName categoryRegistryModule='SurveyManager' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
...
<div class="z-formrow">
    <label for="surveyManagerSorting">{gt text='Sorting'}:</label>
        <select id="surveyManagerSorting" name="sorting">
...
<div class="z-formrow">
    <label for="surveyManagerAmount">{gt text='Amount'}:</label>
        <input type="text" id="surveyManagerAmount" name="amount" maxlength="2" size="10" value="{$amount|default:"5"}" />
</div>

<div class="z-formrow">
    <label for="surveyManagerTemplate">{gt text='Template'}:</label>
        <select id="surveyManagerTemplate" name="template">
...
<div id="customTemplateArea" class="z-formrow z-hide">
    <label for="surveyManagerCustomTemplate">{gt text='Custom template'}:</label>
        <input type="text" id="surveyManagerCustomTemplate" name="customtemplate" size="40" maxlength="80" value="{$customTemplate|default:''}" />
        <span class="z-sub z-formnote">{gt text='Example'}: <em>itemlist_[objectType]_display.tpl</em></span>
</div>

<div class="z-formrow z-hide">
    <label for="surveyManagerFilter">{gt text='Filter (expert option)'}:</label>
        <input type="text" id="surveyManagerFilter" name="filter" size="40" value="{$filterValue|default:''}" />
