{* purpose of this template: response datas rss feed in user area *}
...
{foreach item='responseData' from=$items}
    <item>
        <title><![CDATA[{if isset($responseData.updatedDate) && $responseData.updatedDate ne null}{$responseData.updatedDate|dateformat} - {/if}{$responseData->getTitleFromDisplayPattern()|notifyfilters:'surveymanager.filterhook.responsedatas'}]]></title>
        <link>{modurl modname='SurveyManager' type='user' func='display' ot='responseData' id=$responseData.id fqurl='1'}</link>
        <guid>{modurl modname='SurveyManager' type='user' func='display' ot='responseData' id=$responseData.id fqurl='1'}</guid>
        {if isset($responseData.createdUserId)}
            ...
            <author>{usergetvar name='email' uid=$responseData.createdUserId} ({$cr_name|default:$cr_uname})</author>
        {/if}
        ...
        {if isset($responseData.createdDate) && $responseData.createdDate ne null}
            <pubDate>{$responseData.createdDate|dateformat:"%a, %d %b %Y %T +0100"}</pubDate>
        {/if}
    </item>
{/foreach}
    </channel>
