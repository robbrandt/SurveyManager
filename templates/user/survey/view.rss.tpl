{* purpose of this template: surveys rss feed in user area *}
...
{foreach item='survey' from=$items}
    <item>
        <title><![CDATA[{if isset($survey.updatedDate) && $survey.updatedDate ne null}{$survey.updatedDate|dateformat} - {/if}{$survey->getTitleFromDisplayPattern()|notifyfilters:'surveymanager.filterhook.surveys'}]]></title>
        <link>{modurl modname='SurveyManager' type='user' func='display' ot='survey' id=$survey.id slug=$survey.slug fqurl='1'}</link>
        <guid>{modurl modname='SurveyManager' type='user' func='display' ot='survey' id=$survey.id slug=$survey.slug fqurl='1'}</guid>
        {if isset($survey.createdUserId)}
            ...
            <author>{usergetvar name='email' uid=$survey.createdUserId} ({$cr_name|default:$cr_uname})</author>
        {/if}

        <description>
            <![CDATA[
            {$survey.description|replace:'<br>':'<br />'}
            ]]>
        </description>
        {if isset($survey.createdDate) && $survey.createdDate ne null}
            <pubDate>{$survey.createdDate|dateformat:"%a, %d %b %Y %T +0100"}</pubDate>
        {/if}
    </item>
{/foreach}
    </channel>
