{* purpose of this template: pages rss feed in user area *}
...
{foreach item='page' from=$items}
    <item>
        <title><![CDATA[{if isset($page.updatedDate) && $page.updatedDate ne null}{$page.updatedDate|dateformat} - {/if}{$page->getTitleFromDisplayPattern()|notifyfilters:'surveymanager.filterhook.pages'}]]></title>
        <link>{modurl modname='SurveyManager' type='user' func='display' ot='page' id=$page.id fqurl='1'}</link>
        <guid>{modurl modname='SurveyManager' type='user' func='display' ot='page' id=$page.id fqurl='1'}</guid>
        {if isset($page.createdUserId)}
            ...
            <author>{usergetvar name='email' uid=$page.createdUserId} ({$cr_name|default:$cr_uname})</author>
        {/if}

        <description>
            <![CDATA[
            {$page.description|replace:'<br>':'<br />'}
            ]]>
        </description>
        {if isset($page.createdDate) && $page.createdDate ne null}
            <pubDate>{$page.createdDate|dateformat:"%a, %d %b %Y %T +0100"}</pubDate>
        {/if}
    </item>
{/foreach}
    </channel>
