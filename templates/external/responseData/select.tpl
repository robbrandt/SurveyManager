...
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Response data'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='responseData' from=$items}
            <option value="{$responseData.id}"{if $selectedId eq $responseData.id} selected="selected"{/if}>{$responseData->getTitleFromDisplayPattern()}</option>
        {foreachelse}
