 ...
 */
function surveymanToggleRelatedItemForm(idPrefix)
{
 ...
 */
function surveymanResetRelatedItemForm(idPrefix)
{
    // hide the sub form
    surveymanToggleRelatedItemForm(idPrefix);
 ...
 */
function surveymanCreateWindowInstance(containerElem, useIframe)
{
    var newWindow;
 ...
 */
function surveymanInitInlineWindow(objectType, containerID)
{
    var found, newItem;
    ...
    relationHandler.each(function (relationHandler) {
        ...
        if (relationHandler.prefix === containerID) {
            ...
            relationHandler.windowInstance = surveymanCreateWindowInstance($(containerID), true);
        }
    });
    ...
    if (found === false) {
        ...
        newItem.windowInstance = surveymanCreateWindowInstance($(containerID), true);
 ...
 */
function surveymanRemoveRelatedItem(idPrefix, removeId)
{
 ...
 */
function surveymanSelectRelatedItem(objectType, idPrefix, inputField, selectedListItem)
{
    ...
    removeLink = Builder.node('a', {id: elemPrefix + 'Remove', href: 'javascript:surveymanRemoveRelatedItem(\'' + idPrefix + '\', ' + newItemId + ');'}, 'remove');
    ...
    if (includeEditing === true) {
        ...
        $(elemPrefix + 'Edit').observe('click', function (e) {
            surveymanInitInlineWindow(objectType, idPrefix + 'Reference_' + newItemId + 'Edit');
            e.stop();
        });
    }
    ...
    if (itemIds !== '') {
        if ($F(idPrefix + 'Scope') === '0') {
            ...
            itemIdsArr.each(function (existingId) {
                if (existingId) {
                    surveymanRemoveRelatedItem(idPrefix, existingId);
                }
            });
            itemIds = '';
        } else {
    ...
    surveymanResetRelatedItemForm(idPrefix);
}

/**
 * Initialise a relation field section with autocompletion and optional edit capabilities
 */
function surveymanInitRelationItemsForm(objectType, idPrefix, includeEditing)
{
    ...
    acOptions = {
        ...
        afterUpdateElement: function (inputField, selectedListItem) {
            ...
            surveymanSelectRelatedItem(objectType, idPrefix, inputField, selectedListItem);
        }
    };
    relationHandler.each(function (relationHandler) {
    ...
    $(idPrefix + 'SelectorDoNew').observe('click', function(e) {
        surveymanInitInlineWindow(objectType, idPrefix + 'SelectorDoNew');
        e.stop();
    });
    ...
    itemIdsArr.each(function (existingId) {
        ...
        if (existingId) {
            ...
            $(elemPrefix).observe('click', function (e) {
                surveymanInitInlineWindow(objectType, elemPrefix);
                e.stop();
            });
        }
    });
 ...
 */
function surveymanCloseWindowFromInside(idPrefix, itemId)
{
        ...
        if (relationHandler['prefix'] === idPrefix) {
            ...
            if (itemId > 0) {
                ...
                if (relationHandler.acInstance !== null) {
                    ...
                    // show a message 
                    Zikula.UI.Alert(Zikula.__('Action has been completed.', 'module_surveymanager_js'), Zikula.__('Information', 'module_surveymanager_js'), {
